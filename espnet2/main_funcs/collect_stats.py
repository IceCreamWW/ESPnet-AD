...
@torch.no_grad()
def collect_stats_multi_iter(
    model: AbsESPnetModel,
    name2train_iter: DataLoader and Iterable[Tuple[List[str], Dict[str, torch.Tensor]]],
    name2valid_iter: DataLoader and Iterable[Tuple[List[str], Dict[str, torch.Tensor]]],
    output_dir: Path,
    ngpu: Optional[int],
    log_interval: Optional[int],
    write_collected_feats: bool,
    ...
    # pdb.set_trace()
    npy_scp_writers = {}
    for dic, mode in zip([name2train_iter, name2valid_iter], ["train", "valid"]):
        for loader_name, itr in dic.items():
            if log_interval is None:
                try:
                    log_interval = max(len(itr) // 20, 10)
                except TypeError:
            ...
            # batch_keys and stats_keys are used by aggregate_stats_dirs.py
            with (output_dir / mode / "batch_keys").open("a", encoding="utf-8") as f:
                f.write(
                    "\n".join(filter(lambda x: not x.endswith("_lengths"), batch)) + "\n"
                )
            with (output_dir / mode / "stats_keys").open("a", encoding="utf-8") as f:
                f.write("\n".join(sum_dict) + "\n")
