...
class ESPnetASRMTLModel(AbsESPnetModel):
    ...
    def apply_mask_to_text_in(self, text_in: torch.Tensor, text_in_lengths: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            mask = mask & len_mask
            text_in[mask] = 1 # assume 1 corresponds to <mask> symbol
    ...
    def expand_alignment_to_text_in(
        ...
        bsz = text_in.shape[0]
        text_in_expand = torch.zeros_like(speech) - 1
        for i in range(bsz):
            repeat = F.pad(alignment[i], (0, text.shape[1] - alignment_lengths[i]), 1)
            text_in_expand[i][:repeat.sum()] = text_in[i].repeat_interleave(repeat)

        text_in = text_in_expand
        text_in_lengths = speech_lengths
    ...
    def forward(
        ...
        if "text" in _type:
            ...
            encoder_out, encoder_out_lens = self.encode(_input, _input_lengths)
