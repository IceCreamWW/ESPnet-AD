...
token_type_in=phn      # Tokenization type (char or bpe).
...
if [ "${token_type_in}" = bpe ]; then
    token_list_in="${bpetoken_list}"
elif [ "${token_type_in}" = char ]; then
    token_list_in="${chartoken_list}"
elif [ "${token_type_in}" = word ]; then
    token_list_in="${wordtoken_list}"
elif [ "${token_type_in}" = phn ]; then
    token_list_in="${phntoken_list}"
else
    log "Error: not supported --token_type_in '${token_type_in}'"
    exit 2
fi
...
if [ -z "${asr_stats_dir}" ]; then
    if [ ! -z "${asr_stats_tag}" ]; then
        asr_stats_dir="${expdir}/asr_stats_${asr_stats_tag}"
    else
        if [ "${lang}" != noinfo ]; then
        ...
        asr_stats_dir+="${token_type_in}_in"
        if [ -n "${speed_perturb_factors}" ]; then
            asr_stats_dir+="_sp"
        fi
    fi
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        ## ================================================
        # split key file for speech
        key_file="${_asr_train_dir}/${_scp}"
        split_scps=""
        for n in $(seq "${_nj}"); do
            split_scps+=" ${_logdir}/train.speech.${n}.scp"
        done
        # shellcheck disable=SC2086
        utils/split_scp.pl "${key_file}" ${split_scps}
        ...
        key_file="${_asr_train_dir}/text_in"
        split_scps=""
        for n in $(seq "${_nj}"); do
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_train_mtl \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                ...
                --token_list_in "${token_list_in}" \
                --non_linguistic_symbols "${nlsyms_txt}" \
                --cleaner "${cleaner}" \
                --g2p "${g2p}" \
                --train_data_path_and_name_and_type_and_loader "${_asr_train_dir}/${_scp},speech,${_type},speech" \
                --train_data_path_and_name_and_type_and_loader "${_asr_train_dir}/text,text,text,speech" \
                --train_data_path_and_name_and_type_and_loader "${_asr_train_dir}/text_in,text_in,text,text" \
                --train_data_path_and_name_and_type_and_loader "${_asr_train_dir}/text_in,text_out,text,text" \
                --valid_data_path_and_name_and_type_and_loader "${_asr_valid_dir}/${_scp},speech,${_type},speech_valid" \
                --valid_data_path_and_name_and_type_and_loader "${_asr_valid_dir}/text,text,text,speech_valid" \
                --train_shape_file_and_loader "${_logdir}/train.speech.JOB.scp",speech \
                --train_shape_file_and_loader "${_logdir}/train.text.JOB.scp",text \
                --valid_shape_file_and_loader "${_logdir}/valid.JOB.scp",speech_valid \
                --output_dir "${_logdir}/stats.JOB" \
        ...
        <"${asr_stats_dir}/train/text_in_shape" \
            awk -v N="$(<${token_list_in} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/text_in_shape.${token_type_in}"

        <"${asr_stats_dir}/train/text_out_shape" \
            awk -v N="$(<${token_list} wc -l)" '{ print $0 "," N }' \
            >"${asr_stats_dir}/train/text_out_shape.${token_type}"
    fi
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type_and_loader ${_asr_train_dir}/${_scp},speech,${_type},speech "
            _opts+="--train_data_path_and_name_and_type_and_loader ${_asr_train_dir}/text,text,text,speech "
            _opts+="--train_data_path_and_name_and_type_and_loader ${_asr_train_dir}/text_in,text_in,text,text "
            _opts+="--train_data_path_and_name_and_type_and_loader ${_asr_train_dir}/text_in,text,text,text "
            _opts+="--train_shape_file_and_loader ${asr_stats_dir}/train/speech_shape,speech "
            _opts+="--train_shape_file_and_loader ${asr_stats_dir}/train/text_shape.${token_type},speech "
            _opts+="--train_shape_file_and_loader ${asr_stats_dir}/train/text_in_shape,text "
            _opts+="--train_shape_file_and_loader ${asr_stats_dir}/train/text_out_shape.${token_type},text "
        fi
        ...
        # shellcheck disable=SC2086
            # --dist_master_addr ${MASTER_IP}  \
            # --dist_master_port ${MASTER_PORT} \
            # --dist_init_method "file://${asr_exp}/dist_init_shared" \
        ${python} -m espnet2.bin.asr_train_mtl \
            --multiprocessing_distributed true \
            --ngpu ${ngpu} \
            --dist_rank ${DLTS_ROLE_IDX}  \
            ...
            --valid_data_path_and_name_and_type_and_loader "${_asr_valid_dir}/${_scp},speech,${_type},speech_valid" \
            --valid_data_path_and_name_and_type_and_loader "${_asr_valid_dir}/text,text,text,speech_valid" \
            --valid_shape_file_and_loader "${asr_stats_dir}/valid/speech_shape,speech_valid" \
            --valid_shape_file_and_loader "${asr_stats_dir}/valid/text_shape.${token_type},speech_valid" \
            --resume true \
            --fold_length_and_loader "${_fold_length},speech" \
            --fold_length_and_loader "${asr_text_fold_length},speech" \
            --fold_length_and_loader "${asr_text_fold_length},text" \
            --fold_length_and_loader "${asr_text_fold_length},text" \
            --fold_length_and_loader "${asr_text_fold_length},speech_valid" \
            --fold_length_and_loader "${asr_text_fold_length},speech_valid" \
            --output_dir "${asr_exp}" \
            ${_opts} ${asr_args}
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            if "${use_k2}"; then
              ...
              asr_inference_tool="espnet2.bin.asr_mtl_inference"
            fi
