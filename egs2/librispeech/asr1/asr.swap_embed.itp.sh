...
token_type_in=raw      # Tokenization type (char or bpe).
...
phntoken_list="${token_listdir}"/phn/tokens.mfa.txt
...
if [ -z "${asr_tag}" ]; then
    if [ -n "${asr_config}" ]; then
        asr_tag="$(basename "${asr_config}" .yaml)_${feats_type}_swap_embedding"
    else
        asr_tag="train_${feats_type}_swap_embedding"
    fi
    if [ "${lang}" != noinfo ]; then
...
if [ -z "${asr_stats_dir}" ]; then
    if [ "${lang}" != noinfo ]; then
        asr_stats_dir="${expdir}/asr_stats_${train_set}_swap_embedding_${feats_type}_${lang}_${token_type}"
    else
        asr_stats_dir="${expdir}/asr_stats_${train_set}_swap_embedding_${feats_type}_${token_type}"
    fi
    if [ "${token_type}" = bpe ]; then
        asr_stats_dir+="${nbpe}"
    fi
    asr_stats_dir+="_${token_type_in}_in"
    if [ -n "${speed_perturb_factors}" ]; then
        asr_stats_dir+="_sp"
    fi
fi
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        # shellcheck disable=SC2086
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_swap_embedding_train \
                --collect_stats true \
                --use_preprocessor true \
                --bpemodel "${bpemodel}" \
                --token_type "${token_type}" \
                ...
                --train_data_path_and_name_and_type "${_asr_train_dir}/text_in,text_in,text" \
                --train_data_path_and_name_and_type "${_asr_train_dir}/alignment,alignment,text_int" \
    ...
    if [ ${stage} -le 11 ] && [ ${stop_stage} -ge 11 ]; then
        ...
        if [ "${num_splits_asr}" -gt 1 ]; then
            ...
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/text_in,text_in,text "
            _opts+="--train_data_path_and_name_and_type ${_split_dir}/alignment,alignment,text_in "
            _opts+="--train_shape_file ${_split_dir}/speech_shape "
            _opts+="--train_shape_file ${_split_dir}/text_shape.${token_type} "
            _opts+="--train_shape_file ${_split_dir}/text_in_shape.${token_type_in} "
            _opts+="--train_shape_file ${_split_dir}/alignment_shape "
            _opts+="--multiple_iterator true "
            ...
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/text_in,text_in,text "
            _opts+="--train_data_path_and_name_and_type ${_asr_train_dir}/alignment,alignment,text_int "
            _opts+="--train_shape_file ${asr_stats_dir}/train/speech_shape "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_shape.${token_type} "
            _opts+="--train_shape_file ${asr_stats_dir}/train/text_in_shape.${token_type_in} "
    ...
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            if "${use_k2}"; then
              ...
              asr_inference_tool="espnet2.bin.asr_swap_embedding_inference"
            fi
